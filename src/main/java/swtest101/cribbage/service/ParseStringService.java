...
public class ParseStringService {
	...
	public static final String INVALID_HAND_STRING_ERROR_MSG = "Input Hand String is invalid. It should have ten characters: two for each card first is the rank, second is the suit. ";
	...
	public static final String INVALID_HAND_OF_CARDS_ERROR_MSG = "Input hand is not the right size.";
	...
	private static Integer defineRank(final Character rankChar) {
		...
		if (rankChar == KING_CHAR) {
			...
			throw new InvalidRankException(INVALID_RANK_CHAR_ERROR_MSG
					+ rankChar);
		}
		return rank;
	}
	...
	private static Suit defineSuit(final char suitChar) {
		...
		if (suitChar == CLUBS.firstChar()) {
			...
			throw new InputParseException(INVALID_SUIT_CHAR_ERROR_MSG
					+ suitChar);
		}
		return suit;
	}
	...
	public static List<String> parseToListOfStrings(String handOfCards) {

		List<String> returnList = new ArrayList<String>();

		if (handOfCards.length() != 10) {
			throw new InvalidHandStringException(INVALID_HAND_STRING_ERROR_MSG);
		}

		for (int i = 0; i < 10; i += 2) {
			returnList.add(handOfCards.substring(i, i + 2));
		}

		return returnList;
	}

	public static List<Card> parseToListOfCards(List<String> listOfString) {

		List<Card> returnList = new ArrayList<Card>();
		Card card = new Card();

		if (listOfString.size() != 5) {
			throw new InvalidInputHandException(INVALID_HAND_OF_CARDS_ERROR_MSG);
		}

		for (int i = 0; i < listOfString.size(); ++i) {
			card = parseToCard(listOfString.get(i));
			returnList.add(card);
		}
